// src/ChatSanctionsRusses.tsx - VERSION MODIFIÉE 1/5 + 4/5 + NICKNAME - GRADE CAPITAINE
...
const ChatSanctionsRusses: React.FC<Props> = ({ avatar }) => {
  const [input, setInput] = useState("");
  const [messages, setMessages] = useState<string[]>([]);
  const [loading, setLoading] = useState(false);
  const [communicationStyle, setCommunicationStyle] = useState("neutre");
  
  // ✅ FIXE: Bot dédié à SANCTIONS RUSSES
  const selectedBot = "SANCTIONS RUSSES";
...
const levelData = [{ bot_id: selectedBot, content_orientation: 'intermediate' }];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const currentInput = input; // Sauvegarder l'input
    const newMessages = [...messages, `👤 ${currentInput}`];
    setMessages(newMessages);
    setInput(""); // ✅ VIDAGE IMMÉDIAT pour éviter double envoi
    setLoading(true);
...
const backendUrl = import.meta.env.VITE_BACKEND_URL;
  ...
  assistant: selectedBot
      ...
      console.log("📤 Envoi de la requête SANCTIONS RUSSES avec préférences:", requestData);
...
const botReply = response.data?.response || "Pas de réponse.";
      ...
      console.error("❌ Erreur API Assistant SANCTIONS RUSSES:", error);
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-gray-900 text-black dark:text-white">
      ...
      <div className="flex items-center gap-4 px-4 py-2 border-b border-blue-700 dark:border-blue-300">
        <h2 className="text-2xl font-bold">Capitaine SANCTIONS RUSSES</h2>
      </div>
      ...
      <div className="flex flex-1 bg-gray-50 dark:bg-gray-800">
        ...
        <div className="w-1/5 bg-slate-800/20 border-r border-slate-600/30 flex flex-col items-center justify-start pt-8">
          ...
          <div className="mb-6 w-full px-4">
            <img 
              src={avatar} 
              alt="Capitaine Sanctions Russes" 
          ...
          <div className="text-center px-4">
            <h3 className="text-lg font-bold mb-2">Capitaine SANCTIONS RUSSES</h3>
            ...
            {loading && (
              <div className="flex justify-start">
                <div className="bg-gray-200 dark:bg-gray-700 p-3 rounded-lg">
                  <p className="text-blue-600 dark:text-blue-300">Capitaine SANCTIONS RUSSES vérifie...</p>
                </div>
              </div>
            )}
          </div>
      ...
      <form onSubmit={handleSubmit} className="flex p-4 border-t border-blue-700 dark:border-blue-300">
        <input
          ...
          placeholder="Posez votre question au Capitaine SANCTIONS RUSSES..."
          value={input}
...
export default ChatSanctionsRusses;